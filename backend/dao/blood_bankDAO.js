...
export default class blood_bankDAO{
    static async injectDB(conn){
        if(blood_bank){
            return
        }
        try{
            blood_bank = await conn.db(process.env.PROJECT_NS).collection("blood_banks")
        } catch (e){
            console.error(`Unable to establish a collection handle in blood_bankDAO: ${e}`,)
        }
    }
